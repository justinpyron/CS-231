...
class CaptioningRNN(object):
    ...
    def loss(self, features, captions):
        ...
        # Step (2)
        dW_embed = word_embedding_backward(dout, word_cache)
        grads['W_embed'] = dW_embed

        # Step (1)
        _ , dW_proj, db_proj = affine_backward(dout_h0, h0_cache)
    ...
    def sample(self, features, max_length=30):
        ...
        for t in range(max_length):
            ...
            # Since we're only on 1 time step at a time, don't need time dimension
            x = x.squeeze(axis=1)
            ...
            # Step (4)
            x = np.argmax(scores, axis=1) # predictions for each observation
            captions[:,t] = x # update object that we output
            x = x.reshape(-1,1) # Input to word_embeddings_forward must be 2-D
