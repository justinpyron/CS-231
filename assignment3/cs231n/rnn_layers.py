...
def rnn_step_forward(x, prev_h, Wx, Wh, b):
    ...
    next_h = (np.exp(output) - np.exp(-output))/(np.exp(output) + np.exp(-output))
...
def rnn_step_backward(dnext_h, cache):
    ...
    ##############################################################################
    x, prev_h, next_h, Wx, Wh = cache # unpack cache
...
def rnn_forward(x, h0, Wx, Wh, b):
    ...
    cache = list()
    h = np.zeros((N, T, H))
    prev_h = h0 # initialize hidden state to h0

    for t in range(T):
        next_h, _cache = rnn_step_forward(x[:,t,:], prev_h, Wx, Wh, b)
        h[:,t,:] = next_h # store h state for time t
        prev_h = next_h # update h state
        cache.append(_cache)
    ##############################################################################
    #                               END OF YOUR CODE                             #
    ##############################################################################
...
def rnn_backward(dh, cache):
    ...
    dWh = np.zeros((H, H))
    db = np.zeros(H)
    dprev_h = np.zeros((N,H)) # use this to propagate back through time steps

    for t in reversed(range(T)):
        dprev_h += dh[:,t,:] # accumulate gradients w.r.t. all losses
        _dx, _dprev_h, _dWx, _dWh, _db = rnn_step_backward(dprev_h, cache[t])
        dx[:,t,:] = _dx
        dWx += _dWx
        dWh += _dWh
        db += _db
...
def lstm_step_backward(dnext_h, dnext_c, cache):
    ...
    #############################################################################
    prev_h, prev_c, x, Wx, Wh, i, f, o, g, tanh_next_c = cache  # unpack cache

    # Compute gradient from multiple channels flowing through layer
    total_dc = (dnext_c + (1. - tanh_next_c**2) * o * dnext_h)
    dprev_c = f * total_dc
    di = g * total_dc
    df = prev_c * total_dc
    ...
    # Standard matrix-vector backprop
...
def lstm_backward(dh, cache):
    ...
    N, T, H = dh.shape
    N, D = cache[0][2].shape 
    # cache is a list containing sub-cache tuples
    # Third entry (index 2) in each sub_cache tuple is x
    ...
    # Use these to propagate back through time steps
    dnext_h = np.zeros((N,H))
    dnext_c = np.zeros((N,H))
    
    for t in reversed(range(T)):
        dnext_h += dh[:,t,:]
        _dx, dnext_h, dnext_c, _dWx, _dWh, _db = lstm_step_backward(dnext_h, dnext_c, cache[t])
        dx[:,t,:] = _dx
        dWx += _dWx
        dWh += _dWh
        db += _db
